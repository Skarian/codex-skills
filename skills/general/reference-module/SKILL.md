...
2. Resolve storage from the active `SKILL.md`: find the nearest parent named `skills`; its parent is `<scope-root>`. If unavailable, ask for a storage path.
3. Store references in `<scope-root>/reference-modules/<name>`, where `<name>` is the alias, `<owner>/<repo-name>` when the URL has an owner or group, or the repo basename when it does not. Strip trailing `.git`.
...
5. If the target is missing, clone it. Use remote `HEAD` unless a ref was requested.
...
7. After clone/update/copy, check out the requested ref or remote `HEAD`, preferably detached. If the user chose use as-is, leave it untouched.
